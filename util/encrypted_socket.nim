...
proc getSocket(self: EncSocket): Socket = self.socket
proc getHost(self: EncSocket): string = self.host
proc getPort(self: EncSocket): int = self.port
proc getKey(self: EncSocket): array[2, uint64] = self.key
...
proc listen(self:var EncSocket): EncSocket =
    ...
    var client: Socket
    var address = ""
    self.socket.acceptAddr(client, address)
...
proc send(self:EncSocket, data:string) =
    var res: seq[uint8] = @[]
    for c in data:
        res.add(c.uint8)
    self.send(res)
...
export EncSocket, newEncSocket, connect, listen, send, recv, getSocket, getHost, getPort, getKey
